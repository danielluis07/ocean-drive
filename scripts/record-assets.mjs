...
for await (const file of new Bun.Glob('**/*').scan({cwd:'public',onlyFiles:true})) {
 ...
 if (landmark) {
  assets.push({path, url, kind: 'authored', ...geodata(landmarkRecords.get(landmark[1])?.coastline.retrieved), source: 'scripts/generate-landmarks.ts', transformations: `Deterministic GLB v2 export at the ${landmark[2]} tier: recorded coastline rings projected, scaled and simplified, triangulated, lifted by the recorded elevation grid, shaded by height and slope, with a skirt and a surf band around the shoreline`, essential: true, experience: true, sha256: await hash(path)});
  continue;
 }
 if (illustrative) {
...
for (const path of ['components/voyage/travessia-mark.tsx','lib/ocean-surface.ts','lib/ocean-lighting.ts','lib/landmark-surf.ts']) assets.push({path, kind:'procedural', sourceKind:'project-source', creator:'Ocean Drive project contributors', source:path, retrieved:'2026-09-15', rights:'Project-authored source contribution; no imported artwork', proof:'docs/third-party/project-assets.md', transformations:'Compiled by Next into same-origin HTML/application code', essential:path.includes('surface'), experience:true, sha256:await hash(path)});
