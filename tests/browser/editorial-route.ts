...
export const evidenceRoute = [
  ...
  { order: 4, name: "Convergência", id: "convergencia" },
] as const;
...
export function routeButton(page: Page, station: RouteStation) {
  return page.locator("#rota").getByRole("button", { name: new RegExp(`^0${station.order} ${station.name}`) });
}
...
export async function advanceEditorialSignal(page: Page, station: RouteStation, passage: 2 | 3) {
  const article = page.getByRole("article", { name: station.name, exact: true });
  await settleScroll(page);
  await article.getByRole("button", { name: "Próximo sinal", exact: true }).click();
  await expect(article.getByText(`Sinal ${passage} de 3`, { exact: true })).toBeVisible();
}

export async function readEditorialStation(page: Page, station: RouteStation, confirm: "Continuar expedição" | "Conectar expedição" | null) {
  ...
  if (confirm) await page.getByRole("article", { name: station.name, exact: true }).getByRole("button", { name: confirm, exact: true }).click();
}
...
export async function holdVesselAssets(page: Page) {
  ...
  await page.route("**/models/*.glb", async (route) => { await held; await route.continue(); });
  return release;
}
