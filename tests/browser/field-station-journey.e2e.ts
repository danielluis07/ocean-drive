...
async function sailTo(page: Page, id: StationId) {
  ...
  while (!(await reader.isVisible()) && Date.now() < deadline) {
    ...
    const bearing = Math.atan2(
      station.position[0] - pose.position.x,
      pose.position.z - station.position[2],
    );
    const difference = Math.atan2(
      Math.sin(bearing - pose.heading),
      Math.cos(bearing - pose.heading),
    );
    const next =
  ...
  await expect(
    reader.getByRole("heading", { name: station.name, exact: true }),
  ).toBeVisible();
}
...
async function finishStation(page: Page) {
  ...
  await reader
    ...
    .click();
  await reader
    .getByRole("button", { name: "Continuar expedição", exact: true })
    .click();
  await expect(reader).toBeHidden();
}

test(
  "arrival opens a protected reader and completion reveals both middle destinations",
  { tag: "@critical" },
  async ({ page }) => {
    ...
    await page
      ...
      .click();
    await expect(
      page.getByRole("button", { name: /Pulso de Calor/ }),
    ).toBeDisabled();
    await page
    ...
    await expect(
      reader.getByRole("heading", { name: "Pulso de Calor", exact: true }),
    ).toBeVisible();
    await expect(page.locator("#expedition-announcer")).toHaveText(
      "Pulso de Calor, estação aberta.",
    );
    await reader
      .getByRole("button", { name: "Próximo sinal", exact: true })
      .click();
    ...
    await expect(reader).toBeHidden();
    await expect(
      page.getByRole("button", { name: /Corais sob Estresse/ }),
    ).toBeVisible();
    await expect(
      page.getByRole("button", { name: /Respostas Desiguais/ }),
    ).toBeVisible();
    await expect(page.getByRole("button", { name: /Chegada/ })).toHaveCount(0);
  },
);

for (const firstMiddle of [
  "corais-sob-estresse",
  "respostas-desiguais",
] as const) {
  test(
    `complete Live Experience via ${firstMiddle}, sources, bookmarks, revisit and deliberate synthesis`,
    { tag: "@critical" },
    async ({ page, context }, testInfo) => {
      ...
      expect((await state(page)).vesselCheckpoints.current).toEqual(
        beforeReading.vesselCheckpoints.current,
      );
      expect((await state(page)).pauseState).toBe("paused");
      await reader.locator("summary:visible").click();
      await expect(
        reader.getByText("Limite da evidência", { exact: true }).first(),
      ).toBeVisible();
      const source = reader
        .getByRole("link", { name: /Acessar fonte/ })
        .first();
      const sourceUrl = (await source.getAttribute("href"))!;
      await context.route(
        (url) => url.href === sourceUrl,
        (route) =>
          route.fulfill({
            contentType: "text/html",
            body: "<title>Source boundary</title>Source record",
          }),
      );
      ...
      await reader
        .getByRole("button", { name: "Voltar ao sinal", exact: true })
        .click();
      ...
      await expect(reader).toBeHidden();
      await expect(
        page.getByRole("button", { name: "Retomar expedição", exact: true }),
      ).toBeFocused();
      expect((await state(page)).completedStations).toEqual([]);
      await page
        .getByRole("button", { name: /Primeira estação Pulso de Calor/ })
        .click();
      await expect(page.locator("#pulso-de-calor-signal-2")).toBeFocused();
      await reader
        .getByRole("button", { name: "Próximo sinal", exact: true })
      ...
      await page.screenshot({
        path: testInfo.outputPath(`${mobile ? "mobile" : "desktop"}.png`),
        fullPage: true,
      });
      ...
      expect((await state(page)).vesselCheckpoints).toEqual(
        beforeSwitch.vesselCheckpoints,
      );
      expect((await state(page)).bookmarks).toEqual(beforeSwitch.bookmarks);
      await finishStation(page);
      await expect(page.getByRole("button", { name: /Chegada/ })).toHaveCount(
        0,
      );
      const secondMiddle =
      ...
      await finishStation(page);
      expect((await state(page)).middleOrder).toEqual([
        firstMiddle,
        secondMiddle,
      ]);
      ...
      await expect(
        page.getByRole("button", { name: /Conectar expedição/ }),
      ).toHaveCount(0);
      ...
      await expect(
        page.getByRole("heading", {
          name: "Expedição conectada.",
          exact: true,
        }),
      ).toBeFocused();
      expect((await state(page)).connected).toBe(true);
      await reader
        .getByRole("button", { name: "Revisitar estações", exact: true })
        .click();
      await expect(
        page.getByRole("button", { name: /Concluída · revisite Chegada/ }),
      ).toBeVisible();
      expect(
        await page.evaluate(
