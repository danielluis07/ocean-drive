...
export function inspectVessel(buffer: ArrayBuffer) {
  ...
  for (const mesh of gltf.meshes) for (const primitive of mesh.primitives) {
    if (primitive.mode !== undefined && primitive.mode !== 4) throw new Error("Vessel must use triangles");
  ...
  return {
    triangles, min, max, materials: gltf.materials?.length ?? 0,
