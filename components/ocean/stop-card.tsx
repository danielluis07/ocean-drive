...
import type { Stop } from "@/content/editorial";
...
export default function StopCard({ stop, opening, visible, cueVisible, onOpen, onYieldFocus }: StopCardProps) {
    ...
    <div className={band}>
      <section
        ...
        {opening ? (
          ...
          <>
            <p className="font-mono text-xs tracking-[0.2em] text-foreground/85 uppercase short:text-[0.6875rem]">Parada {stopNumber(stop)}</p>
            ...
            <button
              ...
              onClick={onOpen}
              className="mt-5 inline-flex h-11 items-center short:mt-3 short:h-9 gap-2.5 rounded-full bg-card px-5 text-sm font-semibold text-card-foreground text-shadow-none transition-colors hover:bg-secondary focus-visible:outline-3 focus-visible:outline-offset-2 focus-visible:outline-ring">
